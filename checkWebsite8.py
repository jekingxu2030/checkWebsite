...
from urllib.parse import urlparse  # 用于解析URL域名
...
class WebsiteMonitor(QWidget):
    ...
    def monitor_loop(self, url, interval_err, interval_ok, token, immediate):
        ...
        try:
            ...
            index_extensions = ['index.html', 'index.htm']
            index_ok = False
            for ext in index_extensions:
                try:
                    index_resp = requests.get(
                        url + '/' + ext, timeout=10, allow_redirects=True)
                    if index_resp.status_code == 200:
                        index_ok = True
                        break
                except requests.RequestException:
                    continue
            if resp.status_code == 200 and input_domain == urlparse(
                    resp.url).netloc and index_ok:
                fault_type = "正常"
                msg = (
                ...
                self.update_status(
                    f"[正常] 状态码：{resp.status_code}，系统状态：{fault_type}")
                self.sleep_with_interrupt(interval_ok)
            else:
                ...
                self.update_status(
                    f"[异常] 状态码：{resp.status_code}，故障类型：{fault_type}")
    ...
    def send_dingding(self, token, message, title="网站报警"):
        ...
        try:
            response = requests.post(
                webhook, headers=headers, data=json.dumps(data))
