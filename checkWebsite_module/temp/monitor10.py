...
class Monitor:
    ...
    def check_website(self):
        ...
        for path in paths_to_check:
            try:
                ...
                if resp_status_code == 200 and current_domain == input_domain:
                    cert_info = (
                        self.check_ssl_cert_expiry(input_domain)
                        if scheme == "https"
                        else None
                    )
        ...
        if resp_domain != input_domain:
            ...
            if resp_status_code is None:
                ...
                exception_info = None
                if fault_type.startswith("未知状态码"):
                    exception_info = error_msg
                self.send_dingding(
                    title="网站报警",
                    address=self.url,
    ...
    def check_ssl_cert_expiry(self, hostname, port=443):
    ...
    def report_ok(self, code, cert_info=None):
        ...
        if cert_info:
            if cert_info["valid"]:
                hint += f"，证书有效，{cert_info['days_left']}天后过期"
            else:
                hint += f"，证书异常：{cert_info['error']}"
        ...
        self.callback_status(f"[正常] 状态码：{code}，{hint}")
