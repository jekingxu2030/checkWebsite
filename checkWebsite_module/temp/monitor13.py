...
# # 这里假设 ssl_checker.py 在同目录，且已实现check_ssl_status函数
# import ssl_checker
...
#                     full_url, timeout=30, allow_redirects=True, headers=headers
#                 )
...
#                     cert_check_result = None
#                     if self.url.startswith("https"):
#                         cert_check_result = ssl_checker.check_ssl_status(self.url)

#                     # 证书无效但网页可访问时，正常类但带提醒
#                     if cert_check_result and cert_check_result.get("status") != "valid":
#                         cert_status = cert_check_result.get("status")
#                         detail = ""
#                         if cert_status == "expired":
#                             detail = f"证书已过期，过期时间：{cert_check_result.get('not_after')}"
#                         elif cert_status == "not_yet_valid":
#                             detail = f"证书尚未生效，有效开始时间：{cert_check_result.get('not_before')}"
#                         elif cert_status == "hostname_mismatch":
#                             detail = (
#                                 f"证书主机名不匹配，期望：{cert_check_result.get('expected')}，"
#                                 f"证书包含：{cert_check_result.get('cert_hosts')}"
#                             )
...
#                         self.report_ok(
#                             resp_status_code,
#                             cert_info={"not_after": detail, "status": cert_status},
#                         )
#                     else:
#                         # 证书有效正常报告
...
#             detail = cert_info.get("not_after") or ""
#             if status != "valid":
#                 lines.append(f"证书异常提醒：{detail}")
...
from urllib.parse import urlparse, urljoin
from datetime import datetime
import ssl_checker  # 引入你写好的ssl_checker模块
    ...
    def check_website(self):
        ...
        for path in paths_to_check:
            try:
                ...
                if resp_status_code == 200 and resp_domain == input_domain:
                    cert_info = (
                        ssl_checker.check_ssl_status(self.url)
                        if self.url.startswith("https")
                        else None
                    )
    ...
    def report_ok(self, code, cert_info=None):
        ...
        if cert_info:
            ...
            if status == "valid":
                ...
                if not_after:
                    lines.append(f"证书有效期至：{not_after}")
            else:
                # 证书状态异常但页面正常
                lines.append(f"证书状态异常：{status}")
        ...
        self.send_dingding(
            ...
            system_hint="网站响应正常",
            status_code=code,
            warm_tip=lines[-1] if len(lines) > 4 else None,
        )
        self.callback_status(f"[正常] 状态码：{code}，系统状态：正常")
